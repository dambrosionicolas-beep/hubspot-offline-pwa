...
import { Plus, User, Cloud, CloudOff, Clock } from 'lucide-react';
import { db } from '../db';
import { api } from '../api';
...
export function ContactList() {
    const contacts = useLiveQuery(() => db.contacts.orderBy('updatedAt').reverse().toArray());
    ...
    const handleRefresh = async () => {
        // Manually fetch from API and update local DB (if online)
        if (navigator.onLine) {
            try {
                setSyncProgress({ current: 10, total: 100, message: t('sync.fetching') + ' ' + t('nav.contacts') + '...' });
                const remoteContacts = await api.fetchContacts();

                setSyncProgress({ current: 40, total: 100, message: t('sync.saving') + ' ' + t('nav.contacts') + '...' });
                await db.contacts.bulkPut(remoteContacts);
                ...
                // alert(`Successfully synced ${remoteContacts.length} contacts and ${remoteActivities.length} activities from HubSpot!`);
            } catch (error) {
                console.error('Failed to fetch contacts', error);
                alert('Failed to sync contacts. Check console for details.');
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-md">
                <ul className="divide-y divide-gray-200">
                    {paginatedContacts.map((contact) => (
                        <li key={contact.id}>
                            <Link to={`/edit/${contact.id}`} className="block hover:bg-gray-50">
                                <div className="px-4 py-4 sm:px-6">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center">
                                            ...
                                            <div className="ml-4">
                                                ...
                                                <div className="text-sm text-gray-500">
                                                    {contact.email}
                                                    {contact.phone && <span className="ml-2 text-gray-400">• {contact.phone}</span>}
                                                </div>
                                                {contact.jobtitle && (
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            ...
                                            ) : contact.syncStatus.startsWith('pending') ? (
